...
> Lab project — MSc AI Finance. Full RL pipeline applied to BTC-USD: custom trading environment, tabular Q-learning baseline, Deep Q-Network with experience replay and target network, ablation study, policy analysis, and reward shaping.

> **Reproducibility:** all plots and GIFs in this repo are generated by running `TP_RL_Trading.ipynb`. They are committed once for README rendering; re-running the notebook overwrites them locally but you should not re-commit them unless results meaningfully change.

...
def _episode_reward(agent: DQNAgent, prices: np.ndarray, window: int, cost: float) -> float:
...
def train(
    ...
    for ep in range(n_episodes):
        ...
        # ── Validation check ──
        if (ep + 1) % val_every == 0:
            val_r = _episode_reward(agent, val_prices, window, cost)
...
def plot_training(episode_rewards: list, val_rewards: list, fname: str = "dqn_training.png"):
